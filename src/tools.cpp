...
lmarker Tools::lidarSense(Car& car, pcl::visualization::PCLVisualizer::Ptr& viewer, long long timestamp, bool visualize, bool use_lidar_ukf)
{
    ...
    car.ukf.ProcessMeasurement(meas_package, use_lidar_ukf);
...
// double time:: movement time in seconds to predict
// int frequency:: prediction frequency
void Tools::ukfResults(Car car, pcl::visualization::PCLVisualizer::Ptr& viewer, int time, int frequency)
{
    UKF ukf = car.ukf;
    viewer->addSphere(pcl::PointXYZ(car.ukf.x[0], car.ukf.x[1], 3.5), 0.5, 0, 1, 0, car.name + "_ukf");
    viewer->addArrow(pcl::PointXYZ(car.ukf.x[0], car.ukf.x[1], 3.5), pcl::PointXYZ(car.ukf.x[0] + car.ukf.x[2]*cos(car.ukf.x[3]), car.ukf.x[1] + car.ukf.x[2]*sin(car.ukf.x[3]), 3.5), 0, 1, 0, car.name + "_ukf_vel");
    if(time > 0)
    {
