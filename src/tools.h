...
class Tools {
    ...
    */
    ...
    // Members
    std::vector<float> nis_lidar;
    std::vector<float> nis_radar;
    std::vector<VectorXd> estimations;
    std::vector<VectorXd> ground_truth;
    
    double noise(double stddev, long long seedNum);
    lmarker lidarSense(Car& car, pcl::visualization::PCLVisualizer::Ptr& viewer, long long timestamp, bool visualize, bool use_lidar_ukf);
    rmarker radarSense(Car& car, Car ego, pcl::visualization::PCLVisualizer::Ptr& viewer, long long timestamp, bool visualize);
    void ukfResults(Car car, pcl::visualization::PCLVisualizer::Ptr& viewer, int time, int frequency);
    /**
