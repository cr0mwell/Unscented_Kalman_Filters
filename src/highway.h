...
public:
    ...
    void stepHighway(double egoVelocity, long long timestamp, int frame_per_sec, int prediction_time, int prediction_frequency, pcl::visualization::PCLVisualizer::Ptr& viewer, bool use_lidar_ukf)
    {
        ...
        {
            std::cout << "*****************" << std::endl;
            std::cout << "PROCESSING CAR " << i << std::endl;
            std::cout << "*****************" << std::endl;
            ...
            if(trackCars[i])
            {
                ...
                tools.lidarSense(traffic[i], viewer, timestamp, visualize_lidar, use_lidar_ukf);
                tools.radarSense(traffic[i], egoCar, viewer, timestamp, visualize_radar);
                tools.ukfResults(traffic[i], viewer, prediction_time, prediction_frequency);
                tools.nis_lidar.push_back(traffic[i].ukf.NIS_laser);
                tools.nis_radar.push_back(traffic[i].ukf.NIS_radar);
                VectorXd estimate(4);
            ...
            std::cout << "*************************" << std::endl;
            std::cout << "COMPLETE PROCESSING CAR " << i << std::endl;
            std::cout << "*************************" << std::endl;
        }
        
        float tb_lidar = std::count_if(tools.nis_lidar.begin(), tools.nis_lidar.end(), [](float x) {return x>5.991;});
        float tb_radar = std::count_if(tools.nis_radar.begin(), tools.nis_radar.end(), [](float x) {return x>7.815;});
        viewer->addText("Above NIS threshold (LiDAR): " + std::to_string(tb_lidar*100/tools.nis_lidar.size()), 30, 375, 20, 1, 1, 1, "nis_lidar");
        viewer->addText("Above NIS threshold (RADAR): " + std::to_string(tb_radar*100/tools.nis_radar.size()), 30, 350, 20, 1, 1, 1, "nis_radar");
